...
Populator.prototype._getTrackedEntityAttributeType = function (trackedEntityAttributeID, next) {
  ...
  Request.get({
    ...
    if (this._options.specifiedUniqueTrackedEntityAttributeID && !this._cache.uniqueTrackedEntityAttributeID) {
      this._cache.uniqueTrackedEntityAttributeID = specifiedUniqueTrackedEntityAttributeID;
    }
    // if the unique tracked entity attribute ID is not specified then the first unique ID processed will be used
    else if (!this._cache.uniqueTrackedEntityAttributeID && body.unique === true) {
