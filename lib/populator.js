...
function Populator(options) {
  ...
  if (this._options.uniqueDataElementID && !this._cache.uniqueEventDataElementID) {
    this._cache.uniqueEventDataElementID = this._options.uniqueDataElementID;
  }
...
Populator.prototype._processRow = function (row, next) {
  ...
  // Prevent duplicate event messages being processsed
  if (this._cache.uniqueEventDataElementID) {
    // Check for duplicate events based on the unique data element identifier.
    tasks.splice(4, 0, this._checkForDuplicateEventUsingDataElementID.bind(this, row.dataElements));
...
Populator.prototype._checkForDuplicateEventUsingDataElementID = function (dataElements, trackedEntityInstanceID, duplicate, next) {
  if (typeof duplicate === 'function') {
    next = duplicate;
    duplicate = false;
  }
  var uniqueEventDataElementID = this._cache.uniqueEventDataElementID
  if (!dataElements[uniqueEventDataElementID]) {
    return next(
      new Error('No Data Element with ID: ' + uniqueEventDataElementID)
    )
  }
    ...
    if (body.events && body.events.length > 0) {
      for (var event of body.events) {
        for (var dataValue of event.dataValues) {
          if (dataValue.dataElement === uniqueEventDataElementID) {
            if (dataValue.value === dataElements[uniqueEventDataElementID]) {
              return next(new Error(
                'Duplicate Event "'
                + uniqueEventDataElementID
                + ' - '
                + dataElements[uniqueEventDataElementID]
