...
function orchestrationsInit(thiz, row) {
  if (thiz._options.programID == 'n/a') {
    thiz._options.programID = undefined;
  }
  if (thiz._options.stageID == 'n/a') {
    thiz._options.stageID = undefined;
  }
  if (row.parameters.programDate == '2014-01-01') {
    row.parameters.programDate = undefined;
  }
  if (row.parameters.eventDate == '2014-01-01') {
    row.parameters.eventDate = undefined;
  }
}
...
Populator.prototype._processRow = function (row, next) {
  ...
  var me = this;
  orchestrationsInit(me, row);
  var tasks = [];
  var trackedEntityInstanceId = me._options.trackedEntityInstance;
  if (!trackedEntityInstanceId) {
    // Populate the map of tracked entity attributes to type
    tasks.push(Async.each.bind(null, Object.keys(row.attributes), me._getTrackedEntityAttributeType.bind(me)));
    // Populate the map of data elements to type
    tasks.push(Async.each.bind(null, Object.keys(row.dataElements), me._getDataElementType.bind(me)));
    // Track the entity
    tasks.push(me._addTrackedEntity.bind(me, row.parameters, row.attributes));
  }
  if (!me._options.request || me._options.request == 'ENROLLMENT') {
    // Enroll in the program
    tasks.push(me._enrollInProgram.bind(me, row.parameters));
  }
  if (!me._options.request || ['CURRENT_STAGE_CHANGED', 'CURRENT_STAGE_EDITED'].indexOf(me._options.request) >= 0) {
    if (me._options.duplicateThreshold >= 0) {
      // Check for duplicate events
      if(trackedEntityInstanceId) {
        tasks.push(me._checkForDuplicateEvent.bind(me, row.parameters, trackedEntityInstanceId));
      } else {
        tasks.push(me._checkForDuplicateEvent.bind(me, row.parameters));
      }
    }
    // Track the events
    tasks.push(me._addEvent.bind(me, row.parameters, row.dataElements));
  }
...
Populator.prototype._checkForDuplicateEvent = function (knownKeys, trackedEntityInstanceID, duplicate, next) {
  ...
  var request = Request.get({
    ...
    if (this._options.duplicateStageID || this._options.request == 'CURRENT_STAGE_EDITED') {
...
Populator.prototype._addEvent = function (knownKeys, dataElements, trackedEntityInstanceID, duplicate, next) {
  ...
  var payload = {
    program: this._options.programID,
    programStage: duplicate && this._options.duplicateStageID ? this._options.duplicateStageID : this._options.stageID,
    trackedEntityInstance: trackedEntityInstanceID,
