...
function orchestrationsInit(thiz, row) {
  if (thiz._options.programID == 'n/a' && row.parameters.programDate == '2014-01-01') {
    thiz._options.programID = undefined;
    row.parameters.programDate = undefined;
  }
  if (thiz._options.stageID == 'n/a' && row.parameters.eventDate == '2014-01-01') {
...
Populator.prototype._processRow = function (row, next) {
  ...
  if (this._options.programID && row.parameters.programDate) {
    // Enroll in the program
    tasks.push(this._enrollInProgram.bind(this, row.parameters));
  }
  if (this._options.stageID && row.parameters.eventDate) {
    if (this._options.duplicateThreshold >= 0) {
      // Check for duplicate events
      tasks.push(this._checkForDuplicateEvent.bind(this, row.parameters));
    }
    // Track the events
